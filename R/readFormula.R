readFormula <- function(formula,
    ...
    if (missing(specialArgumentNames)) {
        ...
        if (!(all(match(names(specialArgumentNames),specials,nomatch=FALSE))))
            stop(paste("Mispecified argument specialArgumentNames:",paste(names(specialArgumentNames),collapse=", "),"does not match",specials))
    }
