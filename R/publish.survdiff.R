...
publish.survdiff <- function (x, digits = 4, eps=0.0001,verbose=TRUE,...) {
    ...
    if (length(x$n) == 1) {
        ...
        temp <- c(x$obs, x$exp, z, format.pval(1 - pchisq(x$chisq,1),digits=digits,eps=eps))
        ...
        if (verbose==TRUE){
            ...
            cat("\n Chisq=",
                ...
                format.pval(1 - pchisq(x$chisq,df),digits=digits,eps=eps),
                "\n")
        }
    }
